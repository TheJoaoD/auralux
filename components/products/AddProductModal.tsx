...
import { createProduct, calculateProfitMargin } from '@/lib/services/productService'
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto bg-[#F7F5F2] border-[#A1887F]/20">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-4">
          ...
          {/* Product Name */}
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-[#A1887F] mb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Category */}
            <div>
              <label htmlFor="category_id" className="block text-sm font-medium text-[#A1887F] mb-2">
            ...
            {/* SKU */}
            <div>
              <label htmlFor="sku" className="block text-sm font-medium text-[#A1887F] mb-2">
                SKU <span className="text-[#A1887F]/50 text-xs">(opcional)</span>
              </label>
              <input
                {...register('sku')}
                type="text"
              ...
              />
              {errors.sku && (
                <p className="mt-1.5 text-sm text-red-600">{errors.sku.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Sale Price */}
            <div>
              <label htmlFor="sale_price" className="block text-sm font-medium text-[#A1887F] mb-2">
              ...
              {errors.sale_price && (
                <p className="mt-1.5 text-sm text-red-600">{errors.sale_price.message}</p>
              )}
            </div>

            {/* Cost Price */}
            <div>
              <label htmlFor="cost_price" className="block text-sm font-medium text-[#A1887F] mb-2">
              ...
              {errors.cost_price && (
                <p className="mt-1.5 text-sm text-red-600">{errors.cost_price.message}</p>
              )}
            </div>
          </div>
          ...
          {salePrice > 0 && costPrice >= 0 && (
            <div className="flex items-center gap-2 p-3 bg-white rounded-lg border border-[#A1887F]/20">
              <TrendingUp className={`h-5 w-5 ${profitMargin > 30 ? 'text-[#C49A9A]' : 'text-[#A1887F]'}`} />
              <span className="text-sm font-medium text-[#A1887F]">
                Margem de Lucro:
              </span>
              <span className={`text-lg font-bold ${profitMargin > 30 ? 'text-[#C49A9A]' : 'text-[#A1887F]'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Quantity */}
            <div>
              <label htmlFor="quantity" className="block text-sm font-medium text-[#A1887F] mb-2">
            ...
            {/* Low Stock Threshold */}
            <div>
              <label htmlFor="low_stock_threshold" className="block text-sm font-medium text-[#A1887F] mb-2">
              ...
              {errors.low_stock_threshold && (
                <p className="mt-1.5 text-sm text-red-600">{errors.low_stock_threshold.message}</p>
              )}
            </div>
          </div>
          ...
          {/* Supplier */}
          <div>
            <label htmlFor="supplier" className="block text-sm font-medium text-[#A1887F] mb-2">
              Fornecedor <span className="text-[#A1887F]/50 text-xs">(opcional)</span>
            </label>
            <input
              {...register('supplier')}
              type="text"
            ...
            />
            {errors.supplier && (
              <p className="mt-1.5 text-sm text-red-600">{errors.supplier.message}</p>
            )}
          </div>
